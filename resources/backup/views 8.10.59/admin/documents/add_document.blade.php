@extends('admin.layouts.admin_master')
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">ระบบจัดการเอกสาร {{$division->title}}</div>
        <div class="panel-body">
            {{Form::open([
            'route'=>['admin.documents.store',$division_id],
            'method'=>'POST',
             'files'=>true
            ])}}
            <div class="col-xs-12 col-md-6 form-group">
                {{Form::label('title','ชื่อเอกสาร')}}
                {{Form::text('title',null,['class'=>'form-control','value'=>old('title')])}}
                <div class="text-danger">{{$errors->first('title')}}</div>
            </div>

            <div class="col-xs-12 col-md-6 form-group">
                {{Form::label('category','หมวดหมู่')}}
                {{Form::select('category',$categories,null,['class'=>'form-control','value'=>old('divisions')])}}
                <div class="text-danger">{{$errors->first('category')}}</div>
            </div>
            <div class="clearfix"></div>

            {{--<div class="col-xs-12 col-md-6 form-group">--}}
                {{--{{Form::label('file','ไฟล์')}}--}}
                {{--<div class="input-group">--}}
                    {{--<input id="file" class="file form-control" name="file" type="text" readonly="readonly" onclick="openKCFinder(this)"--}}
                           {{--style="cursor:pointer" />--}}
                    {{--<span class="input-group-addon">--}}
                      {{--<label onclick="openKCFinder(this)">เลือกไฟล์</label>  --}}{{--<input class="btn btn-default" onclick="openKCFinder(this)" type="button" value="เลือกไฟล์">--}}
                    {{--</span>--}}
                {{--</div>--}}
                {{--<div class="text-danger">{{$errors->first('file')}}</div>--}}
            {{--</div>--}}
            <div class="col-xs-12 col-md-6 form-group">
                {{--{{Form::label('file','ไฟล์')}}--}}
                {{--<div class="form-group">--}}
                    {{--<div class="nopadding col-xs-8">--}}
                        {{--<input name="files"  class="form-control image" type="text"/>--}}
                    {{--</div>--}}
                    {{Form::file('file')}}
                <div class="text-danger">{{$errors->first('file')}}</div>
                    {{--<a class="btn btn-default btn-image">เลือกรูปภาพ</a>--}}
                {{--</div>--}}
            </div>
            <div class="col-xs-12 col-md-6">

            </div>
            <div class="clearfix"></div>

            <div class="col-xs-12 col-md-6 for form-group">
                {{Form::submit('อัพโหลด',['class'=>'btn btn-success btn-block'])}}
            </div>
        </div>
    </div>
    @endsection
@section('page-script')
    <script>
        function openKCFinder(field) {
            window.KCFinder = {
                callBack: function(url) {
//                    field.value=url;
//                    $(".file").val = url;
                    document.getElementById("file").value=decodeURIComponent(url);
//                    alert($(document.getElementById('file')).value);
                    window.KCFinder = null;
                }
            };
            window.open('{{ asset('js/kcfinder/browse.php?type=files') }}', 'kcfinder_textbox',
                    'status=0, toolbar=0, location=0, menubar=0, directories=0, ' +
                    'resizable=1, scrollbars=0, width=800, height=600'
            );
//            window.KCFinder=null;
        }
    </script>

    @endsection